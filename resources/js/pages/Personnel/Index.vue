...
const submit = () => {
    form.post(PersonnelController.store().url, {
        preserveScroll: true,
        onSuccess: () => {
            form.reset();
        },
...
const { filters, reset } = useDynamicFilters(
    ...
    {
        controller: PersonnelController, // ✅ Wayfinder
        debounceKeys: ['search'],
        delay: 300,
        persist: true,
    },
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-wrap items-center justify-between gap-4 p-4">
            ...
            <Modal title="Formulaire de nouveau personnel">
                <form @submit.prevent="submit">
                    ...
                    <div class="grid gap-2">
                        ...
                        <Input
                            ...
                            required
                            placeholder="contact du personnel"
                        />
                        <InputError
                            class="mt-2"
            ...
            <TableBody v-if="filteredRows.length > 0">
                <TableRow v-for="row in filteredRows" :key="row.id">
                    ...
                    <TableCell class="flex">
                        <ButtonEdit
                            :href="PersonnelController.edit({ id: row.id }).url"
                        />
                        <ButtonDelete
                            :href="
                                PersonnelController.destroy({ id: row.id }).url
                            "
                        />
                    </TableCell>
                </TableRow>
