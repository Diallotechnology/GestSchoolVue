...
const { filters, reset } = useDynamicFilters(
    ...
    {
        controller: TypeController, // ✅ Wayfinder
        debounceKeys: ['search'],
        delay: 300,
        persist: true,
    },
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-wrap items-center justify-between gap-4 p-4">
            ...
            <Modal title="Formulaire de nouveau type">
                <Form
                    :action="TypeController.store()"
                    method="post"
                    reset-on-success
            ...
            <TableBody v-if="filteredRows.length > 0">
                <TableRow v-for="row in filteredRows" :key="row.id">
                    ...
                    <TableCell class="flex">
                        <ButtonEdit
                            :href="TypeController.edit({ id: row.id }).url"
                        />
                        <ButtonDelete
                            :href="TypeController.destroy({ id: row.id }).url"
                        />
                    </TableCell>
                </TableRow>
            </TableBody>
